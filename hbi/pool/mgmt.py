...
__all__ = ["PoolMaster", "ProcWorker", "ServiceConsumer"]
...
class PoolMaster:
    ...
    def start_pool(self, loop=None):
        ...
        self._pool_hbis_task = loop.create_task(
            HBIC.create_server(
                lambda: runpy.run_module(
                    f"{__package__}._pmgr.__main__", run_name="__hbi_pool_master__"
                ),
                addr=self.team_addr,
                loop=loop,
            )
    ...
    async def _pool_cleanup(self):
        ...
        if self.team_addr is not None:
            try:
                ...
                logger.warning(
                    f"Failed cleaning up pool temp dir: {self.team_addr}", exc_info=True
                )
            self.team_addr = None
    ...
    async def assign_proc(self, consumer: ServiceConsumer):
        ...
        idle_quota = self.hot_back - (
            self._idle_workers.qsize() + len(self._pending_workers)
        )
        if idle_quota > 0:
            logger.debug(
        ...
        if worker is not None and worker.check_alive():
            ...
            if worker.last_session != consumer.session:
                ...
                if worker.last_session is not None:
                    if (
                        self._workers_by_session.get(worker.last_session, None)
                        is worker
                    ):
    ...
    def restart_worker_process(self, err_reason: Optional[str]):
        ...
        if hbi_peer is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Failed disconnecting proc worker for reason {err_reason!s}",
                    exc_info=True,
                )
        ...
        if self.po is None:
            ...
            pid = self.po.pid
            logger.debug(
                f"Restarting proc worker pid={pid} due to reason: {err_reason!s}"
            )
            try:
    ...
    def check_alive(self):
        ...
        if self.hbi_peer is not None and self.hbi_peer.connected:
            ...
            try:
                ...
                logger.error(
                    f"Connection to proc with pid {self.po.pid} seems dead.",
                    exc_info=True,
                )
    ...
    async def prepare_session(self, session: str = None):
        ...
        try:
            async with self.hbi_peer.co() as hbi_peer:
                confirmed_session = await hbi_peer.co_get(
                    rf"""
